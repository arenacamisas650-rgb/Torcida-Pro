...
export default async function handler(req, res) {
  ...
  // ✅ options[].name DEVE ser string simples "Tamanho" — Nuvemshop NAO aceita objeto {pt:...} aqui
  const payload = {
    name:        { pt: nome },
    description: { pt: desc },
    handle:      { pt: handle },
  ...
  if (result.status === 422) {
    ...
    // ✅ Distinguir tipos de 422:
    //   - 'variant values should not be repeated' → duplicata de values[]
    //   - 'sku' / 'already taken' → SKU já existe em outro produto
    //   - outros → handle duplicado ou campo inválido
    if (msg422.includes('variant') && (msg422.includes('repeat') || msg422.includes('duplicat'))) {
      console.error('[CP] 🚨 422 VARIANTES DUPLICADAS — valores repetidos no payload');
      console.error('[CP] Valores enviados:', valoresFinais);
      return res.status(422).json({
        error:            'Variantes duplicadas (values[] repetidos)',
        detail:           detail422,
    ...
    if (msg422.includes('sku') || msg422.includes('already taken') || msg422.includes('already been taken')) {
      console.error('[CP] 🚨 422 SKU DUPLICADO — SKU já existe em outro produto na Nuvemshop');
      console.error('[CP] SKUs enviados:', variants.map(v => v.sku));
      return res.status(422).json({
        error:        'SKU duplicado — já existe em outro produto na Nuvemshop',
        detail:       detail422,
        skus_enviados: variants.map(v => v.sku),
        solucao:      'Os SKUs são gerados com timestamp. Tente novamente.',
      });
    }
