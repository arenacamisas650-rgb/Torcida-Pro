...
function normVariants(raw, handle) {
  ...
  for (const v of arr) {
    ...
    // ✅ Timestamp em base36 garante SKU único entre produtos e entre importações
    //    Evita 422 "SKU already exists" quando a Nuvemshop tem produtos anteriores
    const sku = (v && v.sku) ? String(v.sku).trim() : `${skuBase}-${skuSufixo}-${Date.now().toString(36)}`;
...
export default async function handler(req, res) {
  ...
  // ✅ options[].name DEVE ser string simples — a API Nuvemshop NÃO aceita objeto multilíngue aqui
  // name/description/handle aceitam {pt:...}, mas options[].name deve ser "Tamanho" (string pura)
  const payload = {
    name:        { pt: nome },
    description: { pt: desc },
    handle:      { pt: handle },
  ...
  if (result.status === 422) {
    ...
    // ── Distinguir tipos de 422 pela mensagem real da Nuvemshop ─────────────
    const nsErroReal = (detail422 && (detail422.description || detail422.message))
      || result.text.slice(0, 300);
    console.error('[CP] Mensagem real da Nuvemshop:', nsErroReal);

    // Variantes com values[] duplicados
    if (msg422.includes('variant') && (msg422.includes('repeat') || msg422.includes('duplicat'))) {
      console.error('[CP] 🚨 422 VARIANTES DUPLICADAS');
      return res.status(422).json({
        error: 'Variantes duplicadas (values[] repetidos) — erro real NS: ' + nsErroReal,
        detail: detail422, valores_enviados: valoresFinais, payload_sent: payload,
      });
    }
    // SKU duplicado
    if (msg422.includes('sku') || msg422.includes('already taken') || msg422.includes('already been taken')) {
      console.error('[CP] 🚨 422 SKU DUPLICADO');
      return res.status(422).json({
        error: 'SKU duplicado — erro real NS: ' + nsErroReal,
        detail: detail422, skus_enviados: variants.map(v => v.sku),
        solucao: 'SKUs gerados com timestamp. Tente novamente.',
      });
    }
    // options inválidas (ex: name como objeto em vez de string simples)
    if (msg422.includes('option') || msg422.includes('invalid') || msg422.includes('name')) {
      console.error('[CP] 🚨 422 CAMPO INVÁLIDO (options/name)');
      return res.status(422).json({
        error: 'Campo inválido no payload — erro real NS: ' + nsErroReal,
        detail: detail422, payload_sent: payload,
      });
    }

    // 422 por handle duplicado → buscar, deletar e recriar
    console.log('[CP] 422 provável handle duplicado — buscando produto existente...');
