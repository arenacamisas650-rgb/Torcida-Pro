...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ✅ CORRIGIDO: values são agora objetos { pt: "P" }, não strings
    ...
    if (optionValues.length > 0 && Array.isArray(sanitizado.variants)) {
      for (const v of sanitizado.variants) {
        ...
        // Extrair string do objeto { pt: "P" } ou usar como string se necessário
        const valStr = (typeof val === 'object' && val.pt) ? val.pt : String(val);
        const optionValuesStr = optionValues.map(o => (typeof o === 'object' && o.pt) ? o.pt : String(o));
        
        if (val && !optionValuesStr.includes(valStr)) {
          return res.status(422).json({
            error: `variant.values[0] "${valStr}" não está em options[0].values (${optionValuesStr.join(', ')})`,
            campo: 'variants',
    ...
    // ── Log do payload sanitizado para debug — NOVO FORMATO ─────────
    ...
    console.log('[create-product] options:', JSON.stringify(sanitizado.options, null, 2));
    console.log('[create-product] variants (primeiro item):', JSON.stringify(sanitizado.variants?.[0], null, 2));
    console.log('[create-product] total variants:', sanitizado.variants?.length);
    console.log('[create-product] imagens:', sanitizado.images?.length, 'itens');
    
    // Log detalhado: formato esperado pela Nuvemshop
    console.log('[create-product] FORMATO VALIDADO:');
    console.log('  - options[0].name:', JSON.stringify(sanitizado.options?.[0]?.name));
    console.log('  - options[0].values:', 
      JSON.stringify(sanitizado.options?.[0]?.values?.map(v => ({ 
        type: typeof v, 
        pt: v.pt || v,
        value: v 
      })))
    );
    console.log('  - variants[0].values:', 
      JSON.stringify(sanitizado.variants?.[0]?.values?.map(v => ({ 
        type: typeof v, 
        pt: v.pt || v,
        value: v 
      })))
...
function sanitizarProduto(p) {
  ...
  if (Array.isArray(p.options) && p.options.length > 0) {
    ...
    // ✅ CORRIGIDO: Manter formato de objetos { pt: "P" }, não converter em strings
    const values = Array.isArray(firstOpt.values)
      ? [...new Map(
          firstOpt.values
            .map(v => {
              // Se é objeto { pt: "P" }, extrair o valor
              if (typeof v === 'object' && v !== null && v.pt) {
                return [v.pt.trim().toUpperCase(), { pt: v.pt.trim().toUpperCase() }];
              }
              // Se é string, converter para objeto
              const str = String(v).trim().toUpperCase();
              return str ? [str, { pt: str }] : null;
            })
            .filter(Boolean)
        ).values()]
      : [];
  ...
  // ✅ CORRIGIDO: values devem ser array de OBJETOS { pt: "P" }, não strings
  if (Array.isArray(p.variants) && p.variants.length > 0) {
    ...
    for (const v of p.variants) {
      ...
      if (Array.isArray(v.values) && v.values.length > 0) {
        const val0 = v.values[0];
        // Se é objeto { pt: "P" }, extrair a string
        tamanho = (typeof val0 === 'object' && val0.pt)
          ? val0.pt.trim().toUpperCase()
          : String(val0).trim().toUpperCase();
      } else {
      ...
      const vOut = {};
      // ✅ CORRIGIDO: values deve ser array de OBJETOS, não strings
      vOut.values = [{ pt: tamanho }];
    ...
    if (out.options && out.options[0]) {
      // ✅ Extrair strings dos tamanhos para comparação
      const tamanhosDasVariants = out.variants.map(v => v.values[0].pt);
      const tamanhosDasOptions  = out.options[0].values.map(v => v.pt);
      ...
      for (const t of tamanhosDasVariants) {
        if (!tamanhosDasOptions.includes(t)) {
          out.options[0].values.push({ pt: t });
      ...
      out.options[0].values = out.options[0].values.filter(
        o => tamanhosDasVariants.includes(o.pt)
      );
    }
  }
...
//  ✅ CORRIGIDO: valores podem ser objetos { pt: "P" } ou strings
// ══════════════════════════════════════════════════════════════════════
function validarVariants(variants) {
  ...
  for (const v of variants) {
    ...
    // ✅ CORRIGIDO: Extrair string do objeto { pt: "P" } ou usar como string
    const val = v.values[0];
    const key = (typeof val === 'object' && val !== null && val.pt)
      ? val.pt.toString().trim().toUpperCase()
