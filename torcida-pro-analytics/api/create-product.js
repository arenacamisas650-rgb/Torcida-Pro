...
function sanitizarProduto(p) {
  ...
  if (Array.isArray(p.variants) && p.variants.length > 0) {
    ...
    for (const v of p.variants) {
      ...
      if (Array.isArray(v.values) && v.values.length > 0) {
        ...
        if (v.option1) values.push(extractStringValue(v.option1));
        if (v.option2) values.push(extractStringValue(v.option2));
        if (v.value) values.push(extractStringValue(v.value));
      }
  ...
  if (Array.isArray(out.options)) {
    out.options = out.options.map((opt, index) => {
      const values = Array.isArray(opt.values)
        ? [...new Set(opt.values.map(v => extractStringValue(v)).filter(Boolean))]
        : [];
      const name = typeof opt.name === 'object'
        ? opt.name
        : { pt: String(opt.name || (index === 0 ? 'Tamanho' : 'Personalização')).trim() };
...
function extractStringValue(value) {
  ...
  if (value && typeof value === 'object') {
    if (typeof value.pt === 'string' && value.pt.trim()) return value.pt.trim();
