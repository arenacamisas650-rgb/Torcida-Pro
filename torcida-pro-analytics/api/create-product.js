...
async function pipPublicarNuvemshop(produto, token, storeId, productId = null) {
  ...
  if (productId) {
    ...
    if (result.status === 422) {
      const errorType = classify422(result.json);
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    if (result.status === 422) {
      ...
      if (errorType === 'VARIANT_DUPLICATE') {
        // Não adianta retry — é bug no payload que chegou com duplicatas
        console.error('[NS] 🚨 Duplicata em values[]. Abortando — verifique o frontend.');
        throw new Error(
          `Variantes duplicadas enviadas ao servidor. Valores: ${variants.map(v => v.values[0]).join(', ')}`
        );
      }
