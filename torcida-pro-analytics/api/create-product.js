...
export default async function handler(req, res) {
  ...
  if (result.status === 422) {
    ...
    console.error('[CP] 422 recebido. Detalhe:', JSON.stringify(detail422));
       ...
       // 422 por variantes duplicadas: so abortar se mensagem for EXPLICITAMENTE sobre repeat
    // Nao confundir handle duplicado (que tambem pode ter "duplicate") com variante duplicada
    const isVariantRepeat = (
      msg422.includes('variant values should not be repeated') ||
      msg422.includes('values should not be repeated') ||
      (msg422.includes('variant') && msg422.includes('repeat'))
    );
    if (isVariantRepeat) {
      console.error('[CP] 422 VARIANTES DUPLICADAS confirmado abortando sem deletar');
    ...
    // 422 por handle duplicado ou outro motivo: buscar, deletar e recriar
    console.log('[CP] 422 nao e por variantes, tentando deletar handle e recriar msg=' + msg422.slice(0, 120));
