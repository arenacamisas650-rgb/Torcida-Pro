...
function normVariants(raw) {
  ...
  for (const v of arr) {
    ...
    const item = {
      price:  (!isNaN(p) && p > 0) ? parseFloat(p.toFixed(2)) : 0,  // número, não string
      stock:  parseInt(v.stock) || 100,
      // stock_management REMOVIDO — campo inválido na API Nuvemshop → causa 422
  ...
  return out.length
    ? out
    : [{ price: 0, stock: 100, sku: 'sku-unico', values: ['UNICO'] }];
}
...
async function criarProduto(payload, storeId, token) {
  ...
  // [DIAGNÓSTICO] Loga a resposta completa sempre que não for 2xx
  if (!resp.ok) {
    console.error('❌ RESPOSTA COMPLETA NUVEMSHOP (status=' + resp.status + '):', text);
  }
...
// ── Limitar imagens a 10 (limite da Nuvemshop) ─────────────────────────────
const imagensLimitadas = rawImages.slice(0, 10);
console.log(`🖼️ Imagens a enviar: ${imagensLimitadas.length} de ${rawImages.length} recebidas`);

const payload = {
  name:        { pt: nome },
  description: { pt: desc },
  handle:      { pt: handle },  // ← localizado (Nuvemshop exige igual a name)
  published:   true,

  // Nuvemshop aceita no máximo 10 imagens por produto
  images: imagensLimitadas.map(img => ({ src: typeof img === 'string' ? img : img.src })),

  options: [
    {
      name:   { pt: 'Tamanho' },
      values: optionValues,     // ← strings simples: [PP,P,M,G,GG]
    }
  ],

  // values = array de strings simples, NÃO objetos { pt: val }
  variants: variants,
};
